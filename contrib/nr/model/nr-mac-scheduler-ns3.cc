    ...
    const std::string& direction) const
{
    ...
    NS_LOG_INFO( "ProcessHARQFeedbacks:" << " direction: " <<  direction );
         ...
         /* nothing as increment */)
    {
        ...
        NS_LOG_INFO("Evaluating feedback: " << *harqFeedbackIt << " Size: " << harqInfo->size());
        ...
        // NS_ASSERT(*rvIt < 40);
        // uint8_t maxHarqReTx = m_enableHarqReTx == true ? 9 : 0;

        if (harqFeedbackIt->IsReceivedOk() )
        {
            ueHarqVector.Erase(harqId);
            harqFeedbackIt = harqInfo->erase(harqFeedbackIt);
            NS_LOG_INFO("ProcessHARQFeedbacks: OK Erased processID " << static_cast<uint32_t>(harqId) << " of UE " << rnti
                                            << " direction " << direction);
        }
        else if ( *rvIt == maxHarqReTx)
        {
            ueHarqVector.Erase(harqId); //JSA commented dont erase UE
            harqFeedbackIt = harqInfo->erase(harqFeedbackIt);

            // ueProcess.m_status = HarqProcess::INACTIVE; //JSA add
            // nackReceived = 0; //JSA add 
            // harqFeedbackIt = harqInfo->end(); //JSA add
            NS_LOG_INFO("ProcessHARQFeedbacks:" << *rvIt << " EXP Erased processID " << static_cast<uint32_t>(harqId) << " of UE " << rnti
                                            << " direction " << direction);
        }
        else if (!harqFeedbackIt->IsReceivedOk())
        {
            ...
            NS_LOG_INFO("ProcessHARQFeedbacks: NACK received for UE " << static_cast<uint32_t>(rnti) << " process "
...
NrMacSchedulerNs3::ResetExpiredHARQ(uint16_t rnti, NrMacHarqVector* harq)
{
    ...
    for (auto harqIt = harq->Begin(); harqIt != harq->End(); ++harqIt)
    {
        ...
        NS_LOG_INFO("ResetExpiredHARQ:" << " processID: " << static_cast<uint32_t>(processId) << " m_status: " << process.m_status << " timer: " << Seconds(process.m_timer));

        if (process.m_status == HarqProcess::INACTIVE)
        {
            NS_LOG_INFO("ResetExpiredHARQ: m_status: INACTIVE " << process.m_status);
            continue;
        }
        ...
        {
            ++process.m_timer;
            NS_LOG_INFO("ResetExpiredHARQ: Updated process for UE " << rnti << " number "
        ...
        else
        {
            ...
            NS_LOG_INFO("ResetExpiredHARQ: Erased process for UE " << rnti << " number "
                                                 << static_cast<uint32_t>(processId)
                                                 << " for time limits");
        }
    }
...
{
    NS_LOG_FUNCTION(this);
    NS_LOG_INFO("ScheduleUl: Scheduling invoked for slot " << params.m_snfSf);
    ...
    NS_LOG_INFO("ScheduleUl: Total DCI for UL : " << ulSlot.m_slotAllocInfo.m_varTtiAllocInfo.size()
                                      << " including UL CTRL");
                                ...
                                LteNrTddSlotType type)
{
    ...
    NS_LOG_DEBUG("DoScheduleUl: Scheduling UL " << ulSfn << " UL HARQ to retransmit: " << ulHarqFeedback.size()
    ...
    if (activeUlHarq.size() > 0)
    {
        ...
        NS_LOG_INFO("DoScheduleUl: For the slot " << ulSfn << " reserved " << static_cast<uint32_t>(usedHarq)
    ...
    for (const auto& alloc : allocInfo->m_varTtiAllocInfo)
    {
        for (auto it = activeUlUe.begin(); it != activeUlUe.end(); ++it)
        {
            ...
            for (auto ueIt = ueInfos.begin(); ueIt != ueInfos.end(); /* no incr */)
            {
                ...
                if (GetUeInfoPtr(*ueIt)->m_rnti == alloc.m_dci->m_rnti ) // && alloc.m_dci->m_type != DciInfoElementTdma::SRS)  //)https://gitlab.com/cttc-lena/nr/-/issues/133
                {
                    NS_LOG_INFO("DoScheduleUl: Removed RNTI " << alloc.m_dci->m_rnti
                                                << " from active ue list "
    ...
    if (ulSymAvail > 0 && activeUlUe.size() > 0)
    {
        ...
        NS_LOG_INFO("DoScheduleUl: For the slot " << ulSfn << " reserved " << static_cast<uint32_t>(usedUl)
    ...
    const NrMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    NS_LOG_INFO( "DoSchedUlTriggerReq: "<< " m_ulHarqInfoList.size:" <<  params.m_ulHarqInfoList.size() << " m_ulHarqToRetransmit.size:" << m_ulHarqToRetransmit.size());

    if (params.m_ulHarqInfoList.size() > 0 || m_ulHarqToRetransmit.size() > 0)  // Cuando llego a maxHarqReTx no entra en el if ¿?
    {
