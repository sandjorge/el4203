...
if [ "$flowType" == "QUIC" ]; then
{
    if [ -f $myfolder/tcp-per.txt ]; then
    {
        rm $myfolder/tcp-per.txt
    }
    fi
    if [ -f $myfolder/tcp-per.txt.gz ]; then
    {
        rm $myfolder/tcp-per.txt.gz
    }
    fi
    if [ -f $myfolder/tcp-delay.txt ]; then
    {
        rm $myfolder/tcp-delay.txt
    }
    fi
    if [ -f $myfolder/tcp-delay.txt.gz ]; then
    {
        rm $myfolder/tcp-delay.txt.gz
    }
    fi

    printf "PER..."
    cat $myfolder/$filename |grep /NodeList/$serverID |  grep Quic | awk '\
    {

        if($1=="t"){
            i=index($0,"ns3::QuicHeader");
            s=substr($0,i,length($0)-i+1);
            i=index(s,"PacketNumber");
            if(i>0){
                PN=substr(s,i,length(s)-i+1);
                PN=substr(PN,14,index(PN,"|")-14);
                i=index(s,"Length");
                LE=substr(s,i,length(s)-i+1);
                LE=substr(LE,8,index(LE,"|")-8);
                T[PN]=$2;
                L[PN]=LE;
                ACK[PN]=0;
                Btx[$2]+=LE;
    ...
    cat $myfolder/$filename |grep /NodeList/$serverID | awk '\
    {
        if($1=="t"){
            ...
            UE=tmp[4]-1;
                SN=substr($33,5,length($33)-4);
                    if(delay[SN,UE] != "" ){
                        if(delay[SN,UE]<0) delay[SN,UE]+=$2;
                        else delay[SN,UE]=$2-time[SN,UE];
                    }
            }
        }END{OFS="\t";
        print "Time","UE","seq","rtt";
        for (c in delay) {
            split(c,tmp, SUBSEP);
            SN=tmp[1];
            UE=tmp[2];
            if(delay[c]>0) {print  time[c],UE,SN, delay[c]}
        }
